...
class PPO:
    ...
    :param cost_lr: Cost cearning rate for optimizer, defaults to 1e-3
    :type cost_lr: float, optional
    :param cost_gamma: Cost discount factor, defaults to 0.99
    :type cost_gamma: float, optional
    :param cost_gae_lambda: Lambda for safety critic general advantage estimation, defaults to 0.95
    :type cost_gae_lambda: float, optional
    ...
    :param cost_vf_coef: Cost value function coefficient, defaults to 0.5
    :type cost_vf_coef: float, optional
    ...
    :type safe_critic_network: nn.Module, optional
    :param device: Device for accelerated computing, 'cpu' or 'cuda', defaults to 'cpu'
    ...
    def __init__(
        ...
        assert cost_lr > 0, "Learning rate must be greater than zero."
        assert isinstance(cost_gamma, (float, int)), "Gamma must be a float."
        assert isinstance(cost_gae_lambda, (float, int)), "Lambda must be a float."
        assert cost_gae_lambda >= 0, "Lambda must be greater than or equal to zero."
        assert isinstance(
        ...
        self.safe_critic_network = safe_critic_network
        self.device = device
        ...
        if (
            ...
            and self.critic_network is not None
            and self.safe_critic_network is not None
        ):
            ...
            self.safe_critic = safe_critic_network
            if (
                isinstance(self.actor, (EvolvableMLP, EvolvableCNN))
                and isinstance(self.critic, (EvolvableMLP, EvolvableCNN))
                and isinstance(self.safe_critic, (EvolvableMLP, EvolvableCNN))
            ):
                self.net_config = self.actor.net_config
            elif (
                isinstance(self.actor, MakeEvolvable)
                and isinstance(self.critic, MakeEvolvable)
                and isinstance(self.safe_critic, MakeEvolvable)
            ):
                self.net_config = None
            else:
                assert (
                    False
                ), f"'actor_network' argument is of type {type(actor_network)}, 'critic_network' of type {type(critic_network)}, \
                     'safe_critic_network' of type {type(safe_critic_network)}, \
                     all must be the same type and be of type \\EvolvableMLP, EvolvableCNN or MakeEvolvable"
        ...
        self.optimizer = optim.Adam(
            [
                ...
                {"params": self.safe_critic.parameters(), "lr": self.lr},
            ]
        )
        ...
        if self.accelerator is not None:
            ...
            self.safe_critic = self.safe_critic.to(self.device)
    ...
    def getAction(self, state, action=None, grad=False):
        ...
        if not grad:
            ...
            with torch.no_grad():
                ...
                cost_state_values = self.safe_critic(state).squeeze(-1)
            self.actor.train()
            self.critic.train()
        ...
        if return_tensors:
            return action, action_logprob, dist_entropy, state_values, cost_state_values
        else:
            return (
                ...
                cost_state_values.cpu().data.numpy(),
            )

    def update_lagrange_penalty(self, constraint_violation):
        """Computes Lagrange penalty update"""
        pass
    ...
    def learn(self, experiences, noise_clip=0.5, policy_noise=0.2):
        ...
        with torch.no_grad():
            ...
            next_cost = self.safe_critic(next_state).reshape(1, -1).cpu()
            advantages = _advantage_estimation(
                values, next_value, rewards, dones, self.gamma, self.gae_lambda
            )
            returns = advantages + values
            cost_advantages = _advantage_estimation(
                cost_values,
                next_cost,
                costs,
                dones,
        ...
        cost_advantages = cost_advantages.reshape(-1)
        cost_returns = cost_returns.reshape(-1)
        ...
        cost_advantages = cost_advantages.to(device)
        cost_returns = cost_returns.to(device)
        cost_values = cost_values.to(device)
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, num_samples, self.batch_size):
                ...
                _, log_prob, entropy, value, cost_value = self.getAction(
                ...
                minibatch_cadvs = cost_advantages[minibatch_idxs]
                minibatch_cadvs = minibatch_cadvs - minibatch_cadvs.mean()

                if self.standardise_cost:
                    minibatch_cadvs /= minibatch_cadvs.std() + 1e-8

                constraint_violation = self.cost_limit - cost_returns[minibatch_idxs]
                self.update_lagrange_penalty(constraint_violation)

                minibatch_advs = (
                    minibatch_advs - self.lagrange_penalty * minibatch_cadvs
                )
                ...
                # Value loss
                v_loss = _get_value_loss(
                    value, values, returns, minibatch_idxs, self.clip_coef
                )

                # Cost value loss
                cv_loss = _get_value_loss(
                    cost_value,
                    cost_values,
                    cost_returns,
                    minibatch_idxs,
        ...
        with torch.no_grad():
            ...
            for i in range(loop):
                ...
                for idx_step in range(max_steps):
                    ...
                    action, _, _, _, _ = self.getAction(state)
                    if not hasattr(env, "num_envs"):
                        action = action[0]
...
def _get_value_loss(val, values, rets, mb_idxs, clip_coef):
    """Computes value loss
    ...
    """
    val = val.view(-1)
    v_loss_unclipped = (val - rets[mb_idxs]) ** 2
    v_clipped = values[mb_idxs] + torch.clamp(
        val - values[mb_idxs], -clip_coef, clip_coef
    )
    v_loss_clipped = (v_clipped - rets[mb_idxs]) ** 2
    v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
    v_loss = 0.5 * v_loss_max.mean()
    return v_loss


def _advantage_estimation(vals, next_vals, rews, ds, gamma, gae_lambda):
    """Computes generalized advantage estimates

    :param vals:
    :type vals: torch.tensor
    """
    num_steps = rews.size(0)
    advs = torch.zeros_like(rews).float()
    for t in range(num_steps):
        ...
        for k in range(t, num_steps):
            if k != num_steps - 1:
                ...
                nextvalue = next_vals.squeeze()

            a_t += discount * (rews[k] + gamma * nextvalue * (1.0 - ds[k]) - vals[k])
            discount *= gamma * gae_lambda * (1.0 - ds[k])
